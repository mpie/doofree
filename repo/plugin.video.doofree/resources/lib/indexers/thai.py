...
def listShows(self, catid, page):
    ...
    page_content = data['content'].encode('utf-8')

    # todo: fix pagination
    pages = range(2, 6)

    limatch += re.compile('<figure>(.+?)</a></li>').findall(page_content)
    ...
    if next_page <= len(pages):
        ...
        for page in pages:
            ...
            query = '?action=%s&page=%d&name=%s&catid=%s' % (action, int(page), 'Page ' + str(page_number), catid)
            url = '%s%s' % (sysaddon, query)
            item = control.item('Page ' + str(page_number), iconImage='', thumbnailImage='')
            if addon_fanart is not None:
                item.setProperty('Fanart_Image', addon_fanart)
            item.setInfo(type="Video",
                         infoLabels={"Title": 'Page ' + str(page_number), "OriginalTitle": 'Page ' + str(page_number)})
...
def list_episodes(self, catid, showid, page, image):
    ...
    for episode in self.list:
        ...
        url = '%s%s' % (sysaddon, query)
        item = control.item(name, iconImage=image, thumbnailImage=image)
        if addon_fanart is not None: item.setProperty('Fanart_Image', addon_fanart)
        item.setInfo(type="Video", infoLabels={"Title": name, "OriginalTitle": name})
...
def sourcePage(self, url, name, image):
    ...
    video_file = re.compile('file: "(.+?)"').findall(result)[0]
    video_url = video_file.replace('s.mp4', '.mp4')
    ...
    control.player.play(video_url + '|Referer:' + url, item)
