...
class Crawler:
    ...
    def crawl(self):
        ...
        for item in data['Home']:
            if 'page' in item:
                ...
                if 1 == Crawler.crawlCounter:
                    records = self.findMovies(content)
                    jsonString = self.createJson(records)
                    f = open(item['id'] + '.json', 'w')
                    f.write(jsonString)
                    f.close()
    ...
    def findMovies(self, content):
        ...
        for movie in movies:
            ...
            record = {'id': int (product_id), 'title':title, 'page':seesantv + show[0][0], 'thumbnail':show[0][1]}
            items.append(record)
        fullContent = {'isFolder': True, 'list': items}
        return fullContent
